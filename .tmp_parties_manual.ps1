...
function CallJson($method, $url, $body=$null, $token=$null) {
  $headers = @{}
  if ($token) { $headers['Authorization'] = "Bearer $token" }
  try {
    if ($null -ne $body) {
      $resp = Invoke-RestMethod -Method $method -Uri $url -ContentType 'application/json' -Headers $headers -Body ($body | ConvertTo-Json -Depth 10)
    } else {
      $resp = Invoke-RestMethod -Method $method -Uri $url -Headers $headers
    }
    return [ordered]@{ ok=$true; status=200; body=$resp }
  } catch {
    $status = $_.Exception.Response.StatusCode.value__
    $txt = ''
    try { $sr = New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream()); $txt = $sr.ReadToEnd() } catch {}
    return [ordered]@{ ok=$false; status=$status; raw=$txt }
  }
}
...
foreach($u in $users){
  $r = CallJson 'Post' "$base/register" $u
  if(-not $r.ok){ throw "register failed for $($u.email): $($r.status) $($r.raw)" }
}
...
if(-not $loginA.ok -or -not $loginB.ok -or -not $loginC.ok){ throw 'login failed' }
...
$joinB = CallJson 'Post' "$base/parties/$party1/join" @{message='join pls'} $tb
$pending = CallJson 'Get' "$base/parties/my-pending-requests" $null $ta
$reqB = $pending.body | Select-Object -First 1
...
# 4.3
$p2 = CallJson 'Post' "$base/parties/event/204669" @{title='Party Join'; description='join'; max_members=3} $ta
$party2 = $p2.body.id
...
# 4.4 reject + noncreator
$checks['4.3.1c'] = CallJson 'Post' "$base/parties/$party2/join" @{message='c join'} $tc
$pending2 = CallJson 'Get' "$base/parties/my-pending-requests" $null $ta
$reqC = ($pending2.body | Where-Object { $_.user_id -eq $uidC } | Select-Object -First 1)
...
# 4.5 + 4.6 + 4.8 on party3
$p3 = CallJson 'Post' "$base/parties/event/204669" @{title='Party Invite'; description='inv'; max_members=3} $ta
$party3 = $p3.body.id
...
$invB = CallJson 'Get' "$base/users/me/party-invites" $null $tb
$invBId = $invB.body[0].id
...
$invC = CallJson 'Get' "$base/users/me/party-invites" $null $tc
$invCId = ($invC.body | Where-Object { $_.party_id -eq $party3 } | Select-Object -First 1).id
...
$p4 = CallJson 'Post' "$base/parties/event/204669" @{title='Party Cancel'; description='inv'; max_members=4} $ta
$party4 = $p4.body.id
$i4 = CallJson 'Post' "$base/parties/$party4/invite" @{user_id=$uidC; message='temp'} $ta
$invC4 = CallJson 'Get' "$base/users/me/party-invites" $null $tc
$invC4Id = ($invC4.body | Where-Object { $_.party_id -eq $party4 } | Select-Object -First 1).id
...
# 4.8 token
$p3detail = CallJson 'Get' "$base/parties/by-id/$party3" $null $ta
$token = $p3detail.body.invite_token
$checks['4.8.1'] = if($token){ [ordered]@{ok=$true; status=200; body=@{token=$token}} } else { [ordered]@{ok=$false; status=0; raw='no token'} }
$checks['4.8.2.preview'] = CallJson 'Get' "$base/parties/by-token/$token"
$checks['4.8.2.join'] = CallJson 'Post' "$base/parties/by-token/$token/join" @{} $tb
$checks['4.8.3'] = CallJson 'Get' "$base/parties/by-token/not_a_real_token"

[ordered]@{users=@($uA.email,$uB.email,$uC.email); ids=@($uidA,$uidB,$uidC); parties=@($party1,$party2,$party3,$party4); checks=$checks} | ConvertTo-Json -Depth 9
