...
$ts='1778023336'
$uA=[ordered]@{ email="party.a.$ts@example.com"; password='Password123!' }
$uB=[ordered]@{ email="party.b.$ts@example.com"; password='Password123!' }
$uC=[ordered]@{ email="party.c.$ts@example.com"; password='Password123!' }
$verifySql = "UPDATE users SET is_verified=true, verification_token=NULL WHERE email IN ('$($uA.email)','$($uB.email)','$($uC.email)');"
...
function CallJson($method, $url, $body=$null, $token=$null) {
  ...
  try { if($null -ne $body){$resp=Invoke-RestMethod -Method $method -Uri $url -ContentType 'application/json' -Headers $headers -Body ($body|ConvertTo-Json -Depth 10)} else {$resp=Invoke-RestMethod -Method $method -Uri $url -Headers $headers}; return [ordered]@{ok=$true;status=200;body=$resp} }
  catch { $status=$_.Exception.Response.StatusCode.value__; $txt=''; try{$sr=New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream());$txt=$sr.ReadToEnd()}catch{}; return [ordered]@{ok=$false;status=$status;raw=$txt} }
}
...
$tok=$p3detail.body.invite_token
