...
function MemberRow({
  member, isCreator, myId, partyId, onUpdate,
}: {
  member: PartyMember; isCreator: boolean; myId: number | null;
  partyId: number; onUpdate: () => void;
}) {
  ...
  const handleDecision = async (action: 'accept' | 'reject') => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/parties/${partyId}/members/${member.user_id}/${action}`, {
        method: 'POST',
      ...
      if (res.ok) {
        toast(
          action === 'accept'
            ? `✅ ${member.username} принят в компанию`
            : `❌ ${member.username} отклонён`,
    ...
    <div className={`flex items-center gap-3 p-3 rounded-xl border ${
      ...
      {isCreator && !isMe && member.status === 'pending' && (
        <div className="flex gap-1">
          <button onClick={() => handleDecision('accept')} disabled={loading}
            className="text-xs px-2 py-1 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition disabled:opacity-50">✓</button>
          <button onClick={() => handleDecision('reject')} disabled={loading}
            className="text-xs px-2 py-1 bg-red-400 text-white rounded-lg hover:bg-red-500 transition disabled:opacity-50">✗</button>
        </div>
      )}
    </div>
  );
...
function PartyCard({ party, onUpdate }: { party: Party; onUpdate: () => void }) {
  ...
  // Если пользователь вышел — закрываем чат
  useEffect(() => {
    if (!user) setChatOpen(false);
  }, [user]);
  ...
  const handleClose = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/parties/${party.id}/close`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) { toast('🔒 Набор закрыт', 'info'); onUpdate(); }
    } catch {}
    setLoading(false);
  };
  ...
  return (
    <>
        ...
        <div className="px-5 py-4 flex items-start gap-3 cursor-pointer" onClick={() => setExpanded(v => !v)}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isAcceptedMember && !isCreator && <span className="text-xs bg-emerald-100 text-emerald-600 px-2 py-0.5 rounded-full font-semibold">✅ Участник</span>}
          ...
          <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
            {isCreator && (
              <button onClick={() => setShowEdit(true)} title="Редактировать"
                className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition text-sm">✏️</button>
            )}
          ...
          <div className="px-5 pb-5 border-t border-gray-50">
            <div className="mt-4 space-y-2">
              ...
              <div className={`flex items-center gap-3 p-3 rounded-xl border ${
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-900 truncate">
                    {party.creator_username}{isCreator && <span className="ml-1 text-xs text-indigo-400">(вы)</span>}
                  </p>
                </div>
              ...
              {party.members.map(m => (
                <MemberRow key={m.user_id} member={m} isCreator={isCreator} myId={myId} partyId={party.id} onUpdate={onUpdate} />
              ))}
              {party.members.length === 0 && <p className="text-xs text-gray-400 pl-1">Партиципантов пока нет</p>}
            </div>
            ...
            {/* Чат — только принятым участникам */}
      ...
      {showEdit && <EditPartyModal party={party} onClose={() => setShowEdit(false)} onSaved={onUpdate} />}
    </>
  );
}
...
export default function EventParty({ eventId }: { eventId: string }) {
  ...
  const fetchParties = useCallback(async () => {
    try {
      ...
      // Детекция изменения статуса для текущего пользователя
      // (уведомление когда поллинг замечает что статус заявки изменился)
      if (token) {
        try {
  ...
  // Поллинг каждые 5 секунд — обновляет данные без перезагрузки
  useEffect(() => {
