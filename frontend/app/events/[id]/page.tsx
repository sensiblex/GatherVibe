      ...
      <main className="container mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div>
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                {event.categories.map(cat => (
                  <span key={cat} className="bg-indigo-50 text-indigo-600 text-sm font-medium px-3 py-1 rounded-full capitalize">{cat}</span>
                ))}
              </div>
          ...
          <div className="space-y-5 lg:sticky lg:top-24 self-start">
            ...
            {/* Даты */}
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-4">📅 Даты</h2>
            ...
            {(event.place_title || event.place_address) && (
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-4">📍 Место</h2>
                ...
                {event.place_phone && (
                  <a href={`tel:${event.place_phone}`}
                    className="text-sm text-indigo-600 hover:text-indigo-800 mt-2 block transition">
                    {event.place_phone}
                  </a>
                )}
                {event.lat && event.lon && (
                  <a href={`https://yandex.ru/maps/?pt=${event.lon},${event.lat}&z=16`}
                    ...
                    className="mt-4 flex items-center gap-2 text-sm text-indigo-500 hover:text-indigo-700 font-medium transition">
                    🗺️ Открыть на карте
                  </a>
                )}
              </div>
